--[[
    Plugin Name: customcalls
    Creator: PrizonPony
]]

CreateThread(function() Config.LoadPlugin("customcalls", function(pluginConfig)
    if pluginConfig.enabled then
        ...
        function HandleCivilianCall(type, typeObj, source, args, rawCommand)
            ...
            if args[1] then
                ...
                TriggerEvent('SonoranCAD::customcalls:SendCallApi', isEmergency, caller, callLocation, description, source, nil, nil, type)
        ...
        CreateThread(function()
            ...
            if pluginConfig.enablePanic then
                ...
                RegisterNetEvent('SonoranCAD::customcalls:SendPanicApi')
                AddEventHandler('SonoranCAD::customcalls:SendPanicApi', function()
                    sendPanic(source, true)
                end)
            end
        ...
        AddEventHandler("SonoranCAD::customcalls:CreateCall", function(data)
            local payload = {
            ...
            performApiRequest({payload}, "NEW_DISPATCH", function(response)
                if response:match("NEW DISPATCH CREATED - ID:") then
                    TriggerEvent("SonoranCAD::customcalls:CallCreated", response:match("%d+"))
                else
        ...
        AddEventHandler("SonoranCAD::customcalls:SendPanic", function(playerId)
            ...
            if id then 
                performApiRequest({{['isPanic'] = true, ['apiId'] = id}}, 'UNIT_PANIC', function() 
                    ...
                    TriggerEvent("SonoranCAD::customcalls:PanicSent", playerId)
                end)
            end
        end)
    
        -- Client Call request
        RegisterServerEvent('SonoranCAD::customcalls:SendCallApi')
        AddEventHandler('SonoranCAD::customcalls:SendCallApi', function(emergency, caller, location, description, source, silenceAlert, useCallLocation, type)
            if location == '' then
            ...
            local uid = uuid()
            TriggerEvent("SonoranCAD::customcalls:cadIncomingCall", emergency, caller, location, description, source, uid, type)
            ...
            if Config.apiSendEnabled then
                ...
                performApiRequest({data}, 'CALL_911', function(response) 
                    if response:match("EMERGENCY CALL ADDED ID:") then
                        TriggerEvent("SonoranCAD::customcalls:EmergencyCallAdd", source, response:match("%d+"))
                    end
                end)
            else
